...
export default {
  ...
  getNewsList(searchInfo) {
    try {
      ...
      if (idStr === null || idStr === '') {
        const returnValue = { data: [], totalCount: 0 }
        return { IsSuccess: true, Msg: '获取列表成功', Data: returnValue }
      }
      ...
      const returnValue = { data: pagedNews, totalCount: totalMatchNews.length }
      return { IsSuccess: true, Msg: '获取列表成功', Data: returnValue }
    } catch (e) {
      console.log('get_list_error:', e)
      const returnValue = { data: [], totalCount: 0 }
      return { IsSuccess: false, Msg: '获取列表失败', Data: returnValue }
    }
  },
  /**
   * 新增新闻
  ...
  addNews(info) {
    try {
      ...
      const news_i = { // 新闻实体
        ...
        insertUserID: store.getters.currentUser.ID, // 录入人ID
  ...
  getNewsDetail(id) {
    try {
      ...
      if (newsStr === null || newsStr === '') {
        return { IsSuccess: false, Msg: '参数错误', Data: emptyValue }
      } else {
      ...
      return { IsSuccess: false, Msg: '获取新闻详情失败', Data: emptyValue }
    }
  },
  /**
   * 新增阅读量
  ...
  addReadCount(id) {
    try {
      ...
      return { IsSuccess: false, Msg: '新增阅读量失败', Data: emptyValue }
    }
  },
  /**
   * 发布、取消发布、删除
    ...
    try {
      ...
      if (newsStr === null || newsStr === '') {
        ...
        if (info.type === 'publish') {
          des = '发布'
          news.newsStatus = 1
          news.publishUserID = store.getters.currentUser.ID
          const pu = { ID: store.getters.currentUser.ID, name: store.getters.currentUser.userName, count: 1 }
      ...
      console.log('addReadCount_error:', e)
      return { IsSuccess: false, Msg: '操作失败', Data: emptyValue }
    }
  }
}
