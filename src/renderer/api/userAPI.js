...
export default {
  ...
  login(info) {
    try {
      ...
      if (userInfo.passWord === info.passWord.trim()) {
        return { IsSuccess: true, Msg: '登陆成功', ReturnValue: 'cms_user_' + matchUserID.ID }
      }
  ...
  getUserInfoByToken(token) {
    try {
      ...
      return { IsSuccess: true, Msg: '获取用户信息成功', ReturnValue: JSON.parse(userInfoStr) }
    } catch (e) {
      console.log('login_error:', e)
      return { IsSuccess: false, Msg: '登陆失败' }
    }
