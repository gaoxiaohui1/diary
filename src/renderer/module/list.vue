      ...
      <el-table :data='tableData' border style='width:100%' v-loading="loading1" element-loading-text="玩命加载中">
        <el-table-column :key='fieldInfo.field' v-for='(fieldInfo,index) in lastFields' :label="fieldInfo.label">
          <template scope="scope">
            <router-link :to="{path: '/detail', query: { ID: scope.row['ID'] }}" v-if="fieldInfo.field==='title'">{{scope.row[fieldInfo.field]}}</router-link>
...
export default {
  ...
  methods: {
    getTableData() { // 获取列表
      ...
      const listRes = diaryAPI.getNewsList(this.searchData)
      this.loading1 = false
    ...
    doOperateNews(id, type) { // 操作
      const operateRes = diaryAPI.operateNews({ id: id, type: type })
      Message({
  ...
  created() {
    const typeRes = diaryAPI.getDistinctInfos({ type: 'type', label: '类型' })
    ...
    const puRes = diaryAPI.getDistinctInfos({ type: 'publishUser', label: '发布人' })
    if (puRes.IsSuccess) {
